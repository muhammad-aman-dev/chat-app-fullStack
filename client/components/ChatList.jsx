...
const ChatList = () => {
    ...
    const { isLoadingChatList, allUsers, chatedUsers }=useSelector((state)=>state.chat)
  ...
  return (
    <div className="w-full sm:w-[24%] flex flex-col text-blue-600">
        <div className="flex items-center gap-4 p-3">
            <input type="text" placeholder="Search Chat" className="font-bold outline-1 outline-blue-400 text-blue-600 focus:shadow-md p-1 rounded-2xl shadow-blue-300"/>
        ...
        <p>Online Users[{onlineUsers.length-1}]</p>
        </div>
        <div className="chats flex flex-col gap-1 h-full w-full overflow-y-auto custom-scrollbar">
           {chatedUsers.map((chat)=>{
            const user = allUsers.find((u) => u._id === chat.userId);
            if (!user) return null;

            const isOnline = onlineUsers.includes(user._id);

            return <div className="chat flex h-[70px] p-1 items-center gap-2 hover:bg-gray-100 duration-300">
